...
<body class="gray-bg">
			...
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveStu.htm">
						<input type="hidden" name="id" id="hidden_txt_id1" value=""/>
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
							<td>编号：</td>
								<td><input type="text" id="code1" name="code"
									class="form-control" aria-required="true" required/></td>
	...
	<!-- cc新增和修改对话框 -->
	<div class="modal fade" id="modal_user_cc" role="dialog" aria-labelledby="modal_user_cc" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_cc" method="post" action="reserveCc.htm">
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>课程名：</td>
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-primary"  id="submit_form_cc">保存</button>
							<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
						</div>
					</form>
	...
	$(function () {
	    ...
	    var validator = $("#form_cc").validate({
    		submitHandler: function(form){
   		      $(form).ajaxSubmit({
   		    	dataType:"json",
   		    	success: function (data) {
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	$("#btn_add").click(function(){
	    		...
	    		$("#form_user").resetForm();
	    		document.getElementById("hidden_txt_id1").value='';
	    		$('#modal_user_edit').modal({backdrop: 'static', keyboard: false});
				$('#modal_user_edit').modal('show');
	        });
	    	$("#btn_cc").click(function(){
	    		$("#form_cc").resetForm();
	    		$('#modal_user_cc').modal({backdrop: 'static', keyboard: false});
				$('#modal_user_cc').modal('show');
	        });
	...
	function initEditUser(getSelection){
		$('#hidden_txt_id1').val(getSelection.id);
		$('#courseId').val(getSelection.id);
		$('#name1').val(getSelection.name);
		$('#age').val(getSelection.age);
		$('#code1').val(getSelection.code);
