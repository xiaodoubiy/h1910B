...
<body class="gray-bg">
			  ...
			  <div class="col-lg-2">
				<div class="input-group">
					...
					<select class="form-control" name="txt_search_roleid" id = "txt_search_roleid">
						...
						<c:forEach items="${deList }" var="r">
						 	<option value="${r.deviceid }">${r.status }</option>
						</c:forEach>
                	</select>
				</div>
			 </div>
					...
					<form id="form_user" method="post" action="reserveDevice.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								<td>设备类型：</td>
								<td colspan="4">
									<select class="form-control" name="deviceType" id = "type" aria-required="true" required>
										<option value="">---请选择---</option>
										<option>android_phone</option>
	...
	function initEditUser(getSelection){
		...
		$('#type').val(getSelection.deviceType);
