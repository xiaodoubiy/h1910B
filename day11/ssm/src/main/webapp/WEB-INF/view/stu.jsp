...
<body class="gray-bg">
        ...
        <div class="row">
			  ...
			  <div class="col-lg-2">
				<div class="input-group">
					...
					<select class="form-control" name="courseid" id = "txt_search_courseid">
	...
	<!-- 新增和修改对话框 -->
	<div class="modal fade" id="modal_user_kc" role="dialog" aria-labelledby="modal_user_kc" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_kc" method="post" action="reserveKc.htm">
						<input type="hidden" name="courseid" id="hidden_txt_courseid" value=""/>
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>学生名：</td>
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-primary"  id="submit_form_kc">保存</button>
							<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
						</div>
					</form>
	...
	var TableInit = function () {
	    ...
	    oTableInit.queryParams = function (params) {
	        var temp = {//这里的键的名字和控制器的变量名必须一致，这边改动，控制器也需要改成一样的
	            ...
	            courseid: $("#txt_search_courseid").val(),
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	$("#btn_kc").click(function(){
	    		$('#password').attr("readOnly",false).val(getSelection.password);
	    		$("#form_kc").resetForm();
	    		document.getElementById("hidden_txt_courseid").value='';
	    		$('#modal_user_kc').modal({backdrop: 'static', keyboard: false});
				$('#modal_user_kc').modal('show');
	        });
	    		...
	    		$.ajax({
	    		    ...
	    		    success:function(res){
	    		    	if(res.success){
	    		    		var xary=[];
	    		    		var yary=[];
	    		    		res.data.forEach(function (studentVo) {
								xary.push(studentVo.sname)
	    		    			yary.push({value:studentVo.num,name:studentVo.sname})
